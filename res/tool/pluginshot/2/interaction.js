...
save.onclick = function() {
	var rawValues = bezier.coordinates + '',
		name = prompt('如果你喜欢，你可以给它一个更短的名字。', rawValues);
...
importexport.onsubmit = function() {
	if(this.className === 'import') {
		var overwrite = !confirm('添加到当前曲线？点击“取消”将重写一个新的。');
		
		try {
			var newCurves = JSON.parse(json.value);
		} 
		catch(e) { 
			alert('对不起，这可能是不合法的JSON :('); 
			return false;
		}
...
function updateDelayed() {
	...
	document.title ='贝塞尔曲线设计器 by cubic-bezier.com | ' + bezier;
}